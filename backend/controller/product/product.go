...
func List(ctx iris.Context) {
	...
	if model2.DB.First(&category, cateID).Error != nil {
		SendErrJSON("分类ID不正确", ctx)
		return
	}

	pageSize := config.ServerConfig.PageSize
	queryErr := model2.DB.Offset(offset).Limit(pageSize).Order(orderStr).Find(&products).Error
	...
	for i := 0; i < len(products); i++ {
		err := model2.DB.First(&products[i].Image, products[i].ImageID).Error
		if err != nil {
			fmt.Println(err.Error())
			SendErrJSON("error", ctx)
			return
...
func AdminList(ctx iris.Context) {
	...
	queryErr := model2.DB.Offset(offset).Limit(config.ServerConfig.PageSize).Order(orderStr).Find(&products).Error
...
func save(ctx iris.Context, isEdit bool) {
	...
	var queryProduct model2.Product
	if isEdit {
		if model2.DB.First(&queryProduct, product.ID).Error != nil {
	...
	if isEdit {
		...
		product.BuyCount = 0
		product.TotalSale = 0
		if product.Status != model2.ProductUpShelf && product.Status != model2.ProductDownShelf && product.Status != model2.ProductPending {
			product.Status = model2.ProductPending
		}
	}
	...
	if isEdit && product.Status != model2.ProductUpShelf && product.Status != model2.ProductDownShelf && product.Status != model2.ProductPending {
	...
	for i := 0; i < len(product.Categories); i++ {
		var category model2.Category
		queryErr := model2.DB.First(&category, product.Categories[i].ID).Error
		if queryErr != nil {
	...
	if isEdit {
		...
		if model2.DB.Exec(sql, product.ID).Error != nil {
			saveErr = true
		}
		if model2.DB.Save(&product).Error != nil {
			saveErr = true
		}
	} else {
		if model2.DB.Create(&product).Error != nil {
			saveErr = true
		}
	}
...
func Info(ctx iris.Context) {
	...
	if model2.DB.First(&product, id).Error != nil {
		SendErrJSON("错误的商品id", ctx)
		return
	}

	if model2.DB.First(&product.Image, product.ImageID).Error != nil {
		product.Image = model2.Image{}
	}

	var imagesSQL []uint
	if err := json.Unmarshal([]byte(product.ImageIDs), &imagesSQL); err == nil {
		var images []model2.Image
		if model2.DB.Where("id in (?)", imagesSQL).Find(&images).Error != nil {
			product.Images = nil
		} else {
	...
	if err := model2.DB.Model(&product).Related(&product.Categories, "categories").Error; err != nil {
		fmt.Println(err.Error())
		SendErrJSON("error", ctx)
		return
	}
	...
	if product.HasProperty {
		if err := model2.DB.Model(&product).Related(&product.Properties).Error; err != nil {
			fmt.Println(err.Error())
			SendErrJSON("error", ctx)
			return
		}
		...
		for i := 0; i < len(product.Properties); i++ {
			...
			if err := model2.DB.Model(&property).Related(&property.PropertyValues).Error; err != nil {
				fmt.Println(err.Error())
				SendErrJSON("error", ctx)
				return
			}
		...
		for i := 0; i < len(product.Inventories); i++ {
			...
			if err := model2.DB.Model(&inventory).Related(&inventory.PropertyValues, "property_values").Error; err != nil {
				fmt.Println(err.Error())
				SendErrJSON("error", ctx)
				return
			}
...
func UpdateStatus(ctx iris.Context) {
	...
	var product model2.Product
	if err := model2.DB.First(&product, productID).Error; err != nil {
		SendErrJSON("无效的产品ID.", ctx)
		return
	}

	if status != model2.ProductDownShelf && status != model2.ProductUpShelf && status != model2.ProductPending {
	...
	if err := model2.DB.Save(&product).Error; err != nil {
		SendErrJSON("error.", ctx)
		return
	}
...
func UpdateHasProperty(ctx iris.Context) {
	...
	var product model2.Product
	if err := model2.DB.First(&product, data.ProductID).Error; err != nil {
		fmt.Println(err.Error())
		SendErrJSON("错误的商品id", ctx)
		return
	}
