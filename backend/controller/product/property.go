...
func AddProperty(ctx iris.Context) {
	...
	if err := model2.DB.First(&product, property.ProductID).Error; err != nil {
		fmt.Println(err.Error())
		SendErrJSON("错误的商品id", ctx)
		return
	}

	if err := model2.DB.Create(&property).Error; err != nil {
		fmt.Println(err.Error())
		SendErrJSON("error", ctx)
		return
	}

	property.PropertyValues = []model2.PropertyValue{}
...
func AddPropertyValue(ctx iris.Context) {
	SendErrJSON := common.SendErrJSON
	var productID uint
	var propertyValue model2.PropertyValue
	...
	if len(properties) == 1 {
		var inventory = model2.Inventory{
			ProductID:      productID,
			PropertyValues: append([]model2.PropertyValue{}, propertyValue),
		}
